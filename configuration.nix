...
in
{
  ...
  environment.etc."polyflow/metadata.json.age" =
    let
      fileExists = builtins.pathExists ./metadata.json.age;
      result = if fileExists then "true" else "false";
    in
    builtins.trace "[BUILD] metadata.json.age exists: ${result}"
    (lib.mkIf fileExists {
      source = ./metadata.json.age;
      mode = "0400";  # Read-only for root
    });

  environment.etc."polyflow/sops-key.txt" =
    let
      fileExists = builtins.pathExists ./sops-key.txt;
      result = if fileExists then "true" else "false";
    in
